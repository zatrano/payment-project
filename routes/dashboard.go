...
func registerDashboardRoutes(app *fiber.App) {
	...
	dashboardGroup.Get("/cards/show/:id", cardHandler.ShowCard)
	...
	dashboardGroup.Get("/invitations/show/:id", invitationHandler.ShowInvitation)
	dashboardGroup.Post("/invitations/publish/:id", invitationHandler.PublishInvitation)
	dashboardGroup.Post("/invitations/unpublish/:id", invitationHandler.UnpublishInvitation)

	participantHandler := handlers.NewInvitationParticipantHandler()
	app.Get("/dashboard/invitations/:invitationId/participants", participantHandler.ListParticipants)
	app.Get("/dashboard/invitations/:invitationId/participants/create", participantHandler.ShowCreateForm)
	app.Post("/dashboard/invitations/:invitationId/participants", participantHandler.CreateParticipant)
	app.Delete("/dashboard/invitations/:id/participants/delete", participantHandler.DeleteParticipant)
}
