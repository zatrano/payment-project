...
type InvitationRequest struct {
	...
	IsMultipleParticipant string                  `form:"is_multiple_participant" validate:"required,oneof=true false"`
...
func ParseAndValidateInvitationRequest(c *fiber.Ctx) (InvitationRequest, error) {
	...
	if err := validate.Struct(req); err != nil {
		...
		errorMessages := map[string]string{
			...
			"IsMultipleParticipant_required": "Çoklu katılımcı durumu seçilmelidir.",
			"IsMultipleParticipant_oneof":    "Çoklu katılımcı durumu için geçersiz bir değer seçildi.",
