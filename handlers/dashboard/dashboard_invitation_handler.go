...
func (h *DashboardInvitationHandler) ListInvitations(c *fiber.Ctx) error {
	...
	catParams := queryparams.DefaultListParams()
	catParams.SortBy = "name"
	catParams.OrderBy = "asc"
	catResult, _ := h.categoryService.GetAllCategories(catParams)
	var categories []models.InvitationCategory
	if catResult != nil && catResult.Data != nil {
		categories, _ = catResult.Data.([]models.InvitationCategory)
	}
...
func (h *DashboardInvitationHandler) CreateInvitation(c *fiber.Ctx) error {
	...
	invitation := &models.Invitation{
		...
		IsMultipleParticipant: req.IsMultipleParticipant == "true",
...
func (h *DashboardInvitationHandler) UpdateInvitation(c *fiber.Ctx) error {
	...
	newFileName, _ := filemanager.UploadFile(c, "image", "invitations")
	...
	dateValue, _ := time.Parse("02.01.2006", req.Date)
	...
	existingInvitation.Description = req.Description
	existingInvitation.Note = req.Note
	existingInvitation.IsConfirmed = req.IsConfirmed == "true"
	existingInvitation.IsParticipant = req.IsParticipant == "true"
	existingInvitation.IsMultipleParticipant = req.IsMultipleParticipant == "true"
...
func (h *DashboardInvitationHandler) ShowInvitation(c *fiber.Ctx) error {
	...
	return renderer.Render(c, "dashboard/invitations/show", "layouts/invitation", fiber.Map{
		"Title":      "Davetiye Detayları",
		"Invitation": invitation,
	})
}

// Davetiyeyi yayına al
func (h *DashboardInvitationHandler) PublishInvitation(c *fiber.Ctx) error {
	id, err := strconv.Atoi(c.Params("id"))
	if err != nil {
		_ = flashmessages.SetFlashMessage(c, flashmessages.FlashErrorKey, "Geçersiz davetiye ID'si.")
		return c.Redirect("/dashboard/invitations", http.StatusSeeOther)
	...
	if err := h.invitationService.UpdateInvitationWithRelations(c.UserContext(), invitation); err != nil {
		_ = flashmessages.SetFlashMessage(c, flashmessages.FlashErrorKey, "Yayına alma işlemi başarısız: "+err.Error())
		return c.Redirect("/dashboard/invitations", http.StatusSeeOther)
	}
	_ = flashmessages.SetFlashMessage(c, flashmessages.FlashSuccessKey, "Davetiye yayına alındı.")
	return c.Redirect("/dashboard/invitations", http.StatusSeeOther)
}

// Davetiyeyi yayından kaldır
func (h *DashboardInvitationHandler) UnpublishInvitation(c *fiber.Ctx) error {
	id, err := strconv.Atoi(c.Params("id"))
	if err != nil {
		_ = flashmessages.SetFlashMessage(c, flashmessages.FlashErrorKey, "Geçersiz davetiye ID'si.")
		return c.Redirect("/dashboard/invitations", http.StatusSeeOther)
	...
	invitation.IsConfirmed = false
	if err := h.invitationService.UpdateInvitationWithRelations(c.UserContext(), invitation); err != nil {
		_ = flashmessages.SetFlashMessage(c, flashmessages.FlashErrorKey, "Yayından kaldırma işlemi başarısız: "+err.Error())
		return c.Redirect("/dashboard/invitations", http.StatusSeeOther)
	}
	_ = flashmessages.SetFlashMessage(c, flashmessages.FlashSuccessKey, "Davetiye yayından kaldırıldı.")
	return c.Redirect("/dashboard/invitations", http.StatusSeeOther)
}
...
func (h *DashboardInvitationHandler) renderInvitationFormError(c *fiber.Ctx, template, title string, req any, message string, fallback ...*models.Invitation) error {
	...
	invitation := &models.Invitation{
		...
		IsMultipleParticipant: form.IsMultipleParticipant == "true",
